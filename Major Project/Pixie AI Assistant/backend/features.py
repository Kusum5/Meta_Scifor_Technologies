...
def findContact(query):  
    ...
    try:
        ...
        cursor.execute("SELECT contact_no FROM contacts WHERE LOWER(name) LIKE ? OR LOWER(name) LIKE ?", ('%' + query + '%', query + '%'))
        results = cursor.fetchall()
        print(results[0][0])
...
def chatBot(query):
    ...
    try:
        ...
        print(f"Error: {e}")
        speak("I encountered an issue processing the request.")
        return "Error"